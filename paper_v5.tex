...
\begin{abstract}
    ...
    We extend the method to heterogeneous signal classes with a small template vocabulary, and calibrate its significance against a measured pure-noise null. On simulated sparse multi-band surveys the template shape prior substantially improves period recovery over Lomb-Scargle in the few-epochs-per-band regime, and a bounded demonstration on Zwicky Transient Facility RR~Lyrae recovers periods on par with the strongest baseline methods while exposing the practical limits set by truth-catalog precision and by the excess variance of real photometry relative to Gaussian noise models.
...
with $z_k = c_k - i s_k$), normalised so that ${\rm CC}(0) = 1$ when
$f = g$; under the plain $L^2([0,1])$ norm a unit-energy shape has
$\Vert f \Vert^2 = 1/2$.  The phase maximization is solved exactly
...
pair; the full $ugriz$ set of $98$ shapes, nearly all RRab, is used as
the library (the simulated observations themselves are in $g$ and $r$
only).
...
($H = 8$ throughout this section and the next, apart from the bounded
joint-refinement experiments of Section~\ref{sec:vocab-joint}, which use
$H = 4$), dropping the constant
...
\subsection{Joint refinement of the vocabulary}
\label{sec:vocab-joint}

Both selectors above are \emph{one-shot}: they fix the vocabulary from a clean
library before any target data are seen.  A natural alternative is to refine
the vocabulary \emph{jointly} against the observed population by
expectation--maximization (EM), initialized from the pipeline vocabulary.
Each E-step assigns every source to its best-fitting vocabulary template and
records the recovered frequency (the per-template catalog statistic of
Section~\ref{sec:vocab}); each M-step re-estimates every template as the
amplitude-weighted, orbit-aligned average of the sources assigned to it,
iterating to a held-out early stop.  This is an instance of multireference
alignment, whose pooling advantage grows as the per-epoch signal-to-noise
falls -- so one might expect it to help precisely in the sparse regime that
motivates a template method.

It does not, at least for RR\,Lyrae.  Across three bounded experiments
($H=4$, $K=2$, $g+r$, a $60$-day baseline, the \citet{Sesar_etal_2010}
universe, one seed) the joint-minus-pipeline recovery gap is consistent with
zero at every epoch count.  With the population drawn from the same library
both selectors cluster ($32$ sources), and with a zero-mean per-source Fourier
jitter of $0.15$ at $\langle m\rangle = 19$ ($64$ sources), the gap is
$+0.000$ throughout.  The only lever that gives EM genuine headroom --
withholding half the library so the population shapes are \emph{absent} from
the clustered set ($64$ sources) -- produces a gap of $-0.031$ at $N=4$ epochs
per band (pipeline $0.828$ versus joint $0.797$, within one binomial standard
error of zero) and $+0.000$ for $N \geq 6$.  In that $N=4$ cell the EM did
iterate and improve the held-out \emph{validation} recovery (stopping at
iteration $4$ of $7$) -- the machinery works -- but the gain did not
generalize to the disjoint evaluation population.

Three mechanisms explain the null.  (i) The RR\,Lyrae shape manifold is
low-dimensional and densely sampled: even a disjoint half-library contains a
near medoid for any population shape, so the one-shot vocabulary is already
near-optimal.  (ii) Period recovery is shape-robust -- the fractional-error
metric saturates to unity by $N \gtrsim 6$ even with an imperfect template --
so a better-matched shape has no recovery headroom to capture.  (iii) The
regime is wrong: multireference alignment's pooling advantage is a
low-per-epoch-SNR effect, whereas RR\,Lyrae combine $\sim\!0.5$-mag amplitudes
with $\sim\!0.01$--$0.02$-mag photometry (per-epoch signal-to-noise
$\sim\!25$--$50$), so single-source shape information is already sufficient and
pooling adds nothing.  The ``sparse'' axis here is the epoch \emph{count}, not
the per-epoch signal-to-noise.

These runs are bounded (single seed, $H=4$, one universe), and the EM
implementation itself is exact and independently verified -- the
iteration-invariant trigonometric summations are reused across iterations
without loss of precision.  A fuller evaluation (multiple seeds with error
bars, $H=8$, both libraries, and a genuinely low-signal-to-noise population
such as the empirical ZTF error model of Section~\ref{sec:simval-robust}), and
catalog-scale joint inference across many signal classes, are left to future
work.  For RR\,Lyrae, whose realistic regime is high signal-to-noise, the
one-shot pipeline vocabulary is the recommended default and the negative
result already covers the science case.
...
stated $10^4$-frequency production grid} for every method except GLS,
most strongly for the $H = 8$ and binned ones.  A multi-harmonic peak has width ${\sim}\,{\rm Rayleigh}/H$, so
...
and the curve is flat thereafter.  (One production detail: the greedy
curves saturate their coverage at two to three templates, and beyond
saturation the production driver pads the vocabulary to size $K$ by
ascending library index rather than by the farthest-point rule of
Section~\ref{sec:vocab}, so the greedy points beyond the knee are
conservative; the knee itself, and all PAM points, are unaffected.)  The two panels together give the
...
empirical curve is a factor $1.3$--$3.5$ larger than the synthetic one
...
\section{Demonstration on ZTF observations}\label{sec:ztfdemo}

The simulated validation of Section~\ref{sec:simval} is complemented by a
bounded demonstration on real photometry from the Zwicky Transient Facility
\citep[ZTF;][]{Bellm_etal_2019}: a labelled period-recovery test on RR\,Lyrae
and a false-alarm control on a magnitude-selected field sample.  The aim is to
exercise the method end-to-end on genuine, systematics-laden light curves and
to expose what actually limits it -- not to construct a catalog, which is
deferred to future work.

\subsection{Sample and scoring criterion}
\label{sec:ztfdemo-sample}

The recovery sample comprises $73$ unique stars observed in $g$ and $r$ from
ZTF DR23, with per-star baselines of $4.9$--$7.6$\,yr: $58$ \emph{science}
stars ($30$ RRab, $28$ RRc) with periods from the ZTF catalog of periodic
variables \citep[][DR2-derived]{Chen_etal_2020}, drawn from three fields, and
$12$ \emph{anti-join controls} -- Gaia DR3 RR\,Lyrae
\citep{Clementini_etal_2023} in the same fields that \citet{Chen_etal_2020}
did \emph{not} catalog, with Gaia periods as truth (three further Gaia controls
enter only the all-star totals).  Because the \citet{Chen_etal_2020} periods
derive from an earlier ZTF data release of the same survey, recovering them is
not fully independent; the controls, the Gaia cross-periods available for
$55/58$ science stars, and the anti-join comparison of
Section~\ref{sec:ztfdemo-select} are the mitigations.

A recovered peak must satisfy \emph{both} the community fractional criterion
$|f-f_{\rm t}|/f_{\rm t} < 1\%$ \citep{Stringer_et_al_2019} \emph{and} a
phase-coherence criterion $|f-f_{\rm t}|\,T < 0.5$ cycles over the full
baseline $T$ \citep{Graham_etal_2013}; the top peak is then classified as
exact, harmonic ($2f$, $f/2$), a sampling alias ($\pm1$, $\pm1/365.25$,
$\pm1/354.37$\,cyc\,d$^{-1}$), or a miss.  The coherence term dominates: at
$T\approx7.5$\,yr it demands $|\Delta P|/P \lesssim 9\times10^{-5}$ near
$P=0.5$\,d, far stricter than the $1\%$ convention, and it is what turns
truth-catalog imprecision into nominal misses (below).

\subsection{Period recovery}
\label{sec:ztfdemo-recovery}

Under the strict criterion the pre-adjudication recovery rate on the $58$
science stars is $87.9\%$ for the FTP multi-band statistic and $89.7\%$ for
conditional entropy \citep{Graham_etal_2013b}, with all six methods between
$82.8\%$ and $89.7\%$ (every multi-band method $\geq85\%$).  Inspection of the
nominal misses shows they are dominated not by the periodograms but by the
truth catalogs: for the $12$ stars present in both Chen and Gaia the two truth
periods differ by a median $|\Delta P|/P = 3.6\times10^{-5}$ (maximum
$4.8\times10^{-4}$), enough that $1/12$ falls outside the $0.5$-cycle window
under one catalog but not the other, and single-catalog scoring undercounts
genuine recoveries by ${\approx}5$--$12\%$ with a catalog-specific sign.  A
phase-fold adjudication of the nine flagged stars -- weighted folds, six-method
consensus, and independent Gaia periods -- returns eight lawful cases
(truth-catalog imprecision or an outright Chen period error that Gaia
confirms; five are independently anchored to Gaia) and a single genuine
failure (a magnitude-$20$ control with no coherent signal at any period).  One
lawful case is a resolved Blazhko/period-change candidate
(sideband $\Delta f = 5.4\times10^{-4}$\,cyc\,d$^{-1}$, modulation period
${\approx}5.1$\,yr).  Re-scoring against the adjudicated periods raises the FTP
multi-band, multi-band Lomb--Scargle \citep{Vanderplas+Ivezic_2015}, and
conditional-entropy rates to $58/58 = 100\%$ on science, with the FTP
multi-band statistic and conditional entropy reaching $72/73$ ($98.6\%$)
overall and multi-band Lomb--Scargle $71/73$ ($97.3\%$;
Table~\ref{tab:ztfrecovery}).  Crucially, the plain fractional-only $1\%$
convention reproduces these post-adjudication multi-band numbers \emph{without}
any adjudication: the evidence-driven procedure converges to the community
convention from below rather than manufacturing recoveries.  Across all $438$
(star, method) cells there is not a single day-, year-, or $354$-day-beat
alias classification: over baselines of ${\geq}4.9$\,yr the ZTF
window function does not push any method onto a sampling alias of a real
signal at these brightnesses.

\begin{table*}
\centering
\caption{\label{tab:ztfrecovery}Post-adjudication period-recovery rates on the
ZTF RR\,Lyrae demonstration sample (Wilson $95\%$ intervals), under the strict
fractional-\emph{and}-coherence criterion.  ``Science'' are
\citet{Chen_etal_2020} stars, ``controls'' are the Chen-missed Gaia anti-join
stars (Section~\ref{sec:ztfdemo-select}).}
\footnotesize
\begin{tabular}{lccc}
\hline\hline
Method & science ($n{=}58$) & controls ($n{=}12$) & all ($n{=}73$) \\
\hline
FTP multi-band     & $100\%$ $[93.8,100]$ & $91.7\%$ $[64.6,98.5]$ & $98.6\%$ $[92.6,99.8]$ \\
cond.\ entropy     & $100\%$ $[93.8,100]$ & $91.7\%$ $[64.6,98.5]$ & $98.6\%$ $[92.6,99.8]$ \\
multi-band LS      & $100\%$ $[93.8,100]$ & $83.3\%$ $[55.2,95.3]$ & $97.3\%$ $[90.5,99.2]$ \\
GLS (single band)  & $98.3\%$ $[90.9,99.7]$ & $91.7\%$ $[64.6,98.5]$ & $97.3\%$ $[90.5,99.2]$ \\
FTP (single band)  & $96.6\%$ $[88.3,99.0]$ & $91.7\%$ $[64.6,98.5]$ & $94.5\%$ $[86.7,97.8]$ \\
MHLS ($H{=}8$)     & $91.4\%$ $[81.4,96.3]$ & $91.7\%$ $[64.6,98.5]$ & $91.8\%$ $[83.2,96.2]$ \\
\hline
\end{tabular}
\end{table*}

\subsection{Selection function and the anti-join controls}
\label{sec:ztfdemo-select}

The anti-join controls test whether the demonstration merely re-detects
\citet{Chen_etal_2020}'s own selection.  These Chen-missed Gaia RR\,Lyrae are a
genuinely harder population -- median joint epoch counts of $118$ against
$1758$ for the science stars, reaching $r\approx20$ -- yet $11/12$ are
recovered post-adjudication, a difference from the science rate of $+8.3$
percentage points (Newcombe $95\%$ interval $[-0.9,+35.4]$) that is consistent
with zero.  The single control failure is the magnitude-$20$ star, which shows
no coherent power at its Gaia period in any statistic; the measurable selection
effect is therefore photometric \emph{depth}, not any tuning of the methods to
Chen's selection.  This matches the acquisition-stage finding that Chen's
incompleteness in the densest field is driven by epoch coverage rather than
brightness (Chen catalogs only about one in seven of the Gaia RR\,Lyrae
there, ${\sim}130$ of ${\sim}960$).

\subsection{False-alarm behaviour on real photometry}
\label{sec:ztfdemo-fap}

For a false-alarm control we take the $99$ magnitude-selected
($14$--$18.5$\,mag) field objects of the empirical-error sample
(Section~\ref{sec:simval-robust}), of which $81$ pass an amplitude/known-variable
screen.  Sub-sampled to $N=40$ epochs on the identical grid as the
pure-noise null of Section~\ref{sec:fap}, the screened-clean tail is consistent
with that calibration: five band-light-curves exceed the empirical $99$th
percentile against $2.8$ expected (binomial $P = 0.15$), the mild excess
running in the direction of a known trials mismatch (the real sub-sampled
baselines, median $2631$\,d, exceed the null's $1096$\,d, giving ${\approx}2.4$
times more independent frequencies under pure noise).  The three screened
objects that do exceed the threshold are all flagged variables -- two
catalogued eclipsing binaries recovered at twice their orbital frequency and
one Gaia AGN candidate sitting on the $2$\,cyc\,d$^{-1}$ diurnal comb -- i.e.\
the amplitude screen and the periodogram tail select the same objects: signal
behaving like signal.

At \emph{full} depth (median $626$/$947$ epochs per band), however, essentially
the entire real sample -- clean objects included -- sits far above a
homoscedastic-Gaussian null built with the nominal ZTF error bars ($94$--$98\%$
of clean $g$ light curves exceed its $99$th percentile, against $1\%$ expected).
This is the well-known excess-variance property of real ZTF photometry
(empirical scatter several times the nominal per-point errors at these
magnitudes, plus low-level correlated systematics), not a property of template
fitting: single-band GLS is displaced from its null by the same amount as the
FTP catalog statistic (Kolmogorov--Smirnov distances $0.97$ versus $0.96$ in
$g$).  The operational consequence is that synthetic Gaussian nulls -- even
matched in epoch count, baseline, and grid -- must not be used to set detection
thresholds for full-depth real light curves; an empirical null (e.g.\ shuffled
photometry) is required there, and the analytic calibration of
Section~\ref{sec:fap} is a like-for-like statement at fixed epoch count only.

\subsection{Scope of the demonstration}
\label{sec:ztfdemo-scope}

These rates apply to well-observed ZTF RR\,Lyrae: the sample is
magnitude-selected and the ZTF error model floors near $\sigma\approx0.04$\,mag
past $r\approx18.5$, so faint-end completeness is untested here by construction,
as the one magnitude-$20$ failure illustrates.  The controls' small size
($n=12$) leaves the selection-effect difference bounded rather than precisely
measured, and the post-adjudication rates are conditional on the adjudication
calls (five of eight independently Gaia-anchored; the fractional-only
convention reproduces them without adjudication).  A per-source sparse-regime
saturation panel on these real light curves is left to future work; the
sparse-epoch axis is characterized on simulations in
Section~\ref{sec:simval-nsweep}.
...
A bounded real-survey demonstration of the multi-band FTP against
single-band and multi-band Lomb--Scargle \citep{Vanderplas+Ivezic_2015}
and conditional entropy \citep{Graham_etal_2013b} is presented in
Section~\ref{sec:ztfdemo}, on a labelled sample of ZTF RR\,Lyrae with
\citet{Chen_etal_2020} and Gaia DR3 \citep{Clementini_etal_2023} periods as
truth.  The multi-band FTP recovers periods on par with the strongest
baselines, and the exercise surfaces two practical limits that simulations
alone do not: the precision of the truth catalogs themselves at multi-year
phase coherence, and the excess variance of real photometry relative to the
synthetic Gaussian nulls used to calibrate significance
(Section~\ref{sec:fap}).  A survey-scale catalog application, and the joint
across-class vocabulary inference whose one-population form is characterised in
Section~\ref{sec:vocab-joint}, are left to follow-up work.
...
experiment code.  The ZTF DR23 light curves of the RR\,Lyrae
demonstration sample (Section~\ref{sec:ztfdemo}), the
\citet{Chen_etal_2020} and Gaia DR3 \citep{Clementini_etal_2023}
cross-match and truth-period tables, the per-star phase folds used in
adjudication, and the recovery and false-alarm scoring outputs are
likewise versioned under \texttt{experiments/} with their acquisition
...
a fractional period error of at most $0.28\%$ for $f \geq
...
contamination is at most one source per cell at $N \geq 8$ and at most
three in any $N = 4$ cell (GLS and MHLS, whose absolute rates there are
lowest anyway), $\pm1$\,day$^{-1}$ beats are entirely absent at
$N \geq 8$, and the two scoring
