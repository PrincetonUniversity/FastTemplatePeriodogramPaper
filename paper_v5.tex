...
We note that the fast template periodogram, like the LS periodogram, assumes that the reported
...
sum of squared residuals, $V$, to be zero.

Namely, we obtain that
\begin{align}
0 = \frac{\partial V}{\partial\theta_1} &= 2\sum_iw_i(y_i - \hat{y}_i)\left(-\frac{\partial\hat{y}}{\partial\theta_1}\right)_i\\
...
and
\begin{align}
0 = \frac{\partial V}{\partial\theta_3} &= 2\sum_iw_i(y_i - \hat{y}_i)\left(-\frac{\partial\hat{y}}{\partial\theta_3}\right)_i\\
...
To obtain an expression for the periodogram, $P = 1 - V / V_0$, we first compute $V$

\begin{align}
V &= \sum_i w_i (y_i - \hat{y}_i)^2 \\
