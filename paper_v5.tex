...
\begin{align}
    p(\omega, \theta|\mathcal{D}) &= \frac{p(\mathcal{D}|\omega, \theta)p(\omega, \theta)}{p(\mathcal{D})}\\
                        &\propto p(\omega, \theta) \prod_{i=1}^{N_{\mathrm{obs}}}\mathcal{N}(\hat{y}(t_i; \omega, \theta), \sigma^2_i)
\end{align}

where we use $\mathcal{D} = \{(t_i, y_i, \sigma_i) \,|\, 0 < i \le N_{\mathrm{obs}}\}$ as shorthand for the lightcurve observations (avoiding the symbol $X$, which is conventionally reserved for the design matrix). The logarithm of the posterior is

\begin{align}
    \log p(\omega, \theta|\mathcal{D}) &= -\frac{1}{2}\sum_{i=1}^{N_{\mathrm{obs}}}\left(\frac{y_i - \hat{y}(t_i; \omega, \theta)}{\sigma_i}\right)^2 + \mathrm{const.}\\
...
\begin{align}
    \chi^2(\omega) &= \min_{\theta} \sum_i w_i (y_i - \hat{y}(t_i|\omega, \theta))^2\\
                   &= \min_{\theta}\left(-\frac{2}{W}\log p(\omega, \theta|\mathcal{D}) + \mathrm{const.}\right)\\
                   &= -\frac{2}{W}\max_\theta \log p(\omega, \theta|\mathcal{D}) + \mathrm{const.}
\end{align}
...
\begin{equation}
\label{eq:lsismap}
    P(\omega) = \frac{2}{W\chi_0^2} \max_\theta \log p(\omega, \theta|\mathcal{D}) + \mathrm{const.}
\end{equation}
...
$P(\omega) = p(\omega|\mathcal{D}) \propto \int_{\theta \in \Theta} p(\omega, \theta|\mathcal{D})p(\omega, \theta)$
where here the periodogram power is related to log-probabilities $\log p(\omega|\mathcal{D}, \hat{\theta}(\omega))$
where $\omega$ is the angular frequency, $\mathcal{D}$ is shorthand for observations,
and $\hat{\theta}(\omega)$ are the non-frequency parameters that
maximize $\log p(\omega, \theta|\mathcal{D})$ at a given trial frequency.
