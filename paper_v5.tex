...
\begin{abstract}
    ...
    is an order of magnitude faster than existing algorithms for small problems ($N_{\rm obs}\lesssim 10$ observations) and
...
Thus, in the present implementation we allow for negative amplitude
solutions in the model fit and caution the user to check that the best
fit $\theta_1$ is positive. Going forward, the cost of the positivity
constraint is much smaller than the cost of the unconstrained polynomial
root-finding step, because for each candidate frequency the optimal
$\theta_1 > 0$ solution can be obtained by re-evaluating the periodogram
at the same set of polynomial roots after restricting to those that
produce a positive $\theta_1$, with no additional root-finding required.
We therefore plan to bake this check directly into the public API in
v0.2 of the \texttt{ftperiodogram} library: \texttt{power(\dots)} will
always return the better-fitting solution with $\theta_1 > 0$, making
the \texttt{sgn} flag an internal implementation detail rather than a
user-facing knob.
...
$\chi^2$ surface in $\theta_2$ admits up to $6H{-}1$ stationary points
(the degree of the polynomial in Equation~\ref{eq:finalpoly}), so
...
\begin{enumerate}
    ...
    \item Searching over a small library of templates with independent
          ...
          shared-phase multi-band extension of
...
multi-band photometry of RR\,Lyrae.
